@testitem "CairoMakie svg format" tags = [:integration] begin
    ...
    options = Dict{String,Any}("format" => Dict("execute" => Dict("fig-format" => "svg")))
    QNW.NotebookState.with_test_context(; options) do
        ...
        result = QNW.render_mimetypes(fig, mod, Dict{String,Any}())
...
@testitem "CairoMakie png format" tags = [:integration] begin
    ...
    options = Dict{String,Any}("format" => Dict("execute" => Dict("fig-format" => "png")))
    QNW.NotebookState.with_test_context(; options) do
        mod = QNW.NotebookState.notebook_module()
        QNW.run_package_refresh_hooks()
