function _bson_serialize(io::IO, msg_data::Any)
    BSON.bson(io, Dict{Symbol,Any}(:data => msg_data))
    return nothing
end

function _bson_deserialize(io::IO)
    return BSON.load(io)[:data]
end
...
function _serialize_msg(io::IO, msg_type::UInt8, msg_id::MsgID, msg_data::Any)
    ...
    try
        write(io, msg_type)
        write(io, msg_id)
        Base.invokelatest(_bson_serialize, io, msg_data)
