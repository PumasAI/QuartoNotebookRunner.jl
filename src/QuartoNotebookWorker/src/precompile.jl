...
precompile(
    Tuple{
        typeof(QuartoNotebookWorker.Malt._bson_deserialize),
        QuartoNotebookWorker.Malt.Sockets.TCPSocket,
    },
)
...
if VERSION >= v"1.9"
    ...
    precompile(
        Tuple{
            typeof(Core.kwcall),
            NamedTuple{
                (:file, :line, :cell_options),
                Tuple{String,Int64,Base.Dict{String,Any}},
            },
            typeof(QuartoNotebookWorker.include_str),
            Module,
            String,
        },
    )
...
precompile(
    Tuple{
        typeof(QuartoNotebookWorker.refresh!),
        Base.Dict{String,Any},
        Base.Dict{String,Any},
    },
)
...
PrecompileTools.@compile_workload begin
    for code in ["1 + 1", "println(\"abc\")", "error()", "@info \"info text\" value=1"]
        ...
        io = IOBuffer()
        bson =
            QuartoNotebookWorker.Packages.BSON.bson(io, Dict{Symbol,Any}(:data => result))
        seekstart(io)
